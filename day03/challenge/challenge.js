...
function creerInventaire() {
  ...
  function ajouterObjet(objet) {
    sac.push ( objet);
    console.log (`${objet} ajouté au sac . `) ;
  }

  function voirSac() {
    console.log("Contenu du sac :", sac) ;
  }

  return { ajouterObjet  , voirSac }
}

let monSac = creerInventaire() ;

monSac.ajouterObjet("Épée" ) ;
monSac.ajouterObjet ("Potion") ;
monSac.voirSac() ;
